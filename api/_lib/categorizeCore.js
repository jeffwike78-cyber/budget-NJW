...
// Ask Claude to sort a batch of transactions into envelopes. Returns an array
// of { id, categoryId, confidence }. Shared by the /api/categorize endpoint
// (manual "Auto-categorize" button) and the Plaid sync (auto-categorize on
// import). The Anthropic key stays server-side.
export async function categorizeTransactions({ transactions, categories, apiKey, model }) {
  if (!apiKey) throw new Error('ANTHROPIC_API_KEY is not set.');
  if (!transactions?.length || !categories?.length) return [];

  const client = new Anthropic({ apiKey });
  const useModel = model || process.env.CATEGORIZER_MODEL || DEFAULT_MODEL;
...
// Claude is asked for a bare JSON array, but be forgiving if it wraps it.
